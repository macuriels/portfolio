...
- Even though the analysis leads to a counter-intuitive conclusion - that time on social media and mental health issues are negatively correlated - further analysis would be entailed to determine the exact nature of this relationship.
...
The present project aims to explore this social media-mental health relationship, having as the main question: **How does this relationship between social media use and mental health actually look like?**
...
GWI is a global market research company that annually publishes a flagship report which contains diverse consumer metrics. 

In particular, [GWI's 2019 flagship report](https://www.gwi.com/hubfs/Downloads/2019%20Q1%20Social%20Flagship%20Report.pdf) asks 1.7 million internet users aged 16-64 how many hours they spend connected to social network services during a typical day, yielding results for 45 countries.
...
Besides the data on mental health and social media use, we will add socioeconomic indicators into the mix. 

In particular, we will use World Bank indicators - [total population](https://data.worldbank.org/indicator/SP.POP.TOTL) and [gross domestic product (GDP)](https://data.worldbank.org/indicator/NY.GDP.MKTP.CD) - which are ingested via the `wbstats` package.
...
{{< spoiler text="*Click to view the R code used for the plot below*" >}}
...
{{< spoiler text="*Click to view an alternative to the Tableau Viz using R code*" >}}
...
Prevalence of mental disorders has remained stable at around 16% of the population, versus social media use that has gone from 100 minutes per day to 140. Also worth noting that both social media use and mental disorder prevalence observed their steepest increases from 2014-2016.

Lastly, even though mental disorder prevalence has changed over years, changes occur in the hundredths or thousands range at a decimal point (which still equates to a change in thousands or millions in terms of population).

## Data Over The Years by Country - World Map

Still focusing on the trends in the main data, the graphic below is a world map that is meant to explore the hours people spend on social media, as well as the prevalence of mental disorders, by country over the years.
...
```r
# plotting averages over the years by country
...
From the world map we can see the change in data available on social media use for certain countries in Sub-Saharan Africa, versus the data available on mental disorders which remains constant over the years.

This change in data occurs both in terms of availability as well as amount, which is expected given that the global average on mental disorder prevalence also remains relatively constant at around 16%.

Another interesting finding is that, throughout the years, countries in Africa, Latin America and the Caribbean, set themselves as the ones where people seem to spend the most time on social networking sites, but these countries also seem to have a lesser rate of mental disorders. 

In contrast, countries North America and Europe seem to make less use of social media but have a higher presence of mental disorders.
...
As expected, we see data points moving away from the zero-line on the vertical axis which corresponds to data being added for social media use (e.g., Colombia is only moved away from the zero-line until 2018). 
...
This suggests that there is an association between spending more time on social media and a potentially lower rate of mental disorders. 
...
This research leads to a counter-intuitive conclusion: **that social media is weakly and negatively correlated with mental disorders**.
...
However bold the research findings may be, **it is worth highlight that correlation does not imply causation.**

While the negative correlation suggests that there is a tendency for countries where people spend more time on social media to have a lower rate of mental disorders, other factors may be influencing this relationship.

It is crucial to consider various variables and potential confounding factors that could affect both social media use and mental health outcomes.

Additionally, this study is conducted at a country-level, and such high levels of aggregations entail caution since it depends on data being recollected correctly at several levels. 
...
While these are significant limitations, this is a step forward in understanding the relationship between social media and mental health, and sets a common ground for future explorations.